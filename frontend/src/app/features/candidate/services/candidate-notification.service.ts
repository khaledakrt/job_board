...
export class CandidateNotificationService {
  ...
  private readonly apiUrl = `${environment.apiUrl}/candidate/notifications`;
  ...
  refresh(): Observable<
    ApiResponse<CandidateNotification[]> & { meta?: { unreadCount: number } }
  > {
    this.loading.set(true);
    return this.http
      .get<ApiResponse<CandidateNotification[]> & { meta?: { unreadCount: number } }>(
        this.apiUrl
      )
      .pipe(
        tap({
  ...
  markAsRead(id: string): Observable<ApiResponse<CandidateNotification>> {
    return this.http.patch<ApiResponse<CandidateNotification>>(`${this.apiUrl}/${id}/read`, {}).pipe(
      tap({
        next: () => {
